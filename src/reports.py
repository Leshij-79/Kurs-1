...
def spending_by_category(transactions: pd.DataFrame, category: str, date: Optional[str] = None) -> pd.DataFrame:
    """
    Функция подготовки отчёта по транзакциям в выбранной категории за три месяца от выбраной даты
    ...
    filtered_days_start_date = filtered_category[
        start_date <= pd.to_datetime(filtered_category["Дата операции"], dayfirst=True, format="%d.%m.%Y %H:%M:%S")
    ]
    filtered_days = filtered_days_start_date[
        pd.to_datetime(filtered_days_start_date["Дата операции"], dayfirst=True, format="%d.%m.%Y %H:%M:%S")
        <= end_date
    ]
